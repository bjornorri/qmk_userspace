...
#define MAX_TAP_HOLD_KEYS 8 // Up to 8 custom keys

static tap_hold_key_t keys[MAX_TAP_HOLD_KEYS];
static uint8_t        key_count = 0;

void tap_hold_register_key(uint16_t keycode, action_fn_t on_tap, action_fn_t on_hold) {
    if (key_count < MAX_TAP_HOLD_KEYS) {
        keys[key_count++] = (tap_hold_key_t){keycode, on_tap, on_hold, false, false, 0};
    }
}
